<template>
  <div class="pay-main">
    <el-button type="primary" icon="el-icon-s-goods">测试</el-button>
...
export default {
  ...
  methods: {
    getPayInfo() {
      reqPayInfo(this.orderId).then((res) => {
        // console.log(res);
        if (res.code == 200) {
    ...
    async open() {
      ...
      // 弹出框一弹就得请求接口知晓支付状态,若支付成功则关闭弹窗
      // 如果没有定时器则开启一个
      if (!this.timer) {
        // 用this.timer接收一个定时器是因为将来有可能清空
        this.timer = setInterval(() => {
          reqPayStatus(this.orderId).then((res) => {
            // console.log(res);
            if (res.code == 205) {
              // 已经支付，清空定时器，保存支付成功返回的code
              clearInterval(this.timer);
              this.timer = null;
              this.code = res.code;
              this.$msgbox.close();
